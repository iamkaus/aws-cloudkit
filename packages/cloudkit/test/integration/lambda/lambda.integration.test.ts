...
describe('Lambda integration', () => {
  ...
  beforeAll(async () => {
    ...
    const zipFile = await readFile(new URL('./fixtures/lambda.zip', import.meta.url));
    ...
    expect(response.FunctionName).toBe(AWS_TEST_FUNCTION);
  ...
  it('gets the Lambda function', async () => {
    ...
    expect(response.Configuration?.FunctionName).toBe(AWS_TEST_FUNCTION);
  ...
  it('lists the Lambda function', async () => {
    ...
    const functionExists = response.Functions?.some(
      (lambda) => lambda.FunctionName === AWS_TEST_FUNCTION,
    );
  ...
  it('updates the Lambda function code', async () => {
    const zipFile = await readFile(new URL('./fixtures/lambda.zip', import.meta.url));
  ...
  it('deletes the Lambda function', async () => {
