...
import { createSQSQueue } from '../../../src/sqs-service/createQueue.js';
...
import { deleteSQSQueue } from '../../../src/sqs-service/deleteQueue.js';
...
describe('SQS integration', () => {
  ...
  it('sends, receives, changes visibility, and deletes a message', async () => {
    // -----------------------------
    // SEND MESSAGE
    // -----------------------------
    ...
    expect(message?.MessageId).toBe(sendResponse.MessageId);

    expect(message?.Body).toBe('CloudKit SQS integration test');
    ...
    const visibilityResponse = await changeMessageVisibility({
      client,
      QueueUrl: queueUrl,
  ...
  it('sends and deletes messages in batches', async () => {
    // -----------------------------
    // SEND MESSAGE BATCH
    // -----------------------------
    ...
    expect(deleteResponse.Successful).toHaveLength(entries.length);
  }, 60_000);
